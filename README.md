...
| 방법 | 소스 | 결과 |
|------|------|------|
| **폴더로 생성** | 이미지 폴더 | 파일명 → 값 이름, EXIF/스텔스/메타 태그 추출 후 공통 태그 제거 → 값 태그 |
...
| 상태 | 의미 |
|------|------|
| **OK** | 처리 성공. 분류(이동)에서는 상위 변수만 연속 매치돼도 `부분 분류`로 OK가 될 수 있음 |
| **UNKNOWN** | 매칭 실패. 파일명 변경은 건너뜀, 분류는 첫 변수부터 매치 실패 시 건너뜀 |
| **CONFLICT** | 다중 매치 충돌. 파일명 변경은 건너뜀, 분류는 첫 변수부터 충돌 시 건너뜀 |
| **ERROR** | 파일 읽기 실패 등 예외 발생 |

> 분류(이동) 탭은 변수 순서 기준으로 상위부터 연속 매치된 prefix 깊이까지만 폴더를 만들어 부분 분류할 수 있습니다.
...
> 분류(이동)에서 예를 들어 `chara,emotion` 순서일 때 `chara`만 매치되고 `emotion`이 미매치/충돌이면 `chara` 폴더까지만 부분 분류됩니다.
